...
internal sealed class EfLabelStore<TContext> : ILabelStore where TContext : DbContext
{
    ...
    public async Task<Label> CreateAsync(string name, string? color = null, Guid? parentId = null, CancellationToken ct = default)
    {
        ...
        var existing = await _db.Set<Label>().FirstOrDefaultAsync(l => l.Name == name, ct);
        if (existing is not null)
            throw new InvalidOperationException($"標籤 '{name}' 已存在(Id: {existing.Id})。");
    ...
    public Task<Label?> FindAsync(string name, CancellationToken ct = default)
        => _db.Set<Label>().FirstOrDefaultAsync(l => l.Name == name, ct);
    ...
    public async Task RenameAsync(Guid labelId, string newName, CancellationToken ct = default)
    {
        ...
        var label = await _db.Set<Label>().FindAsync(new object[] { labelId }, ct)
            ?? throw new InvalidOperationException($"找不到標籤 {labelId}。");

        var taken = await _db.Set<Label>().AnyAsync(l => l.Name == newName && l.Id != labelId, ct);
        if (taken)
            throw new InvalidOperationException($"標籤名稱 '{newName}' 已被使用。");
    ...
    public async Task DeleteAsync(Guid labelId, CancellationToken ct = default)
    {
        ...
        var hasChildren = await _db.Set<Label>().AnyAsync(l => l.ParentId == labelId, ct);
        if (hasChildren)
            throw new InvalidOperationException(
                $"標籤 '{label.Name}' 尚有子標籤,請先刪除或移動子標籤。(資料庫層亦以 Restrict 外鍵擋下)");

        _db.Set<Label>().Remove(label);
        await _db.SaveChangesAsync(ct);   // 各型別的連結由 cascade delete 自動清除
    }
        ...
        string labelName, bool includeDescendants = true, CancellationToken ct = default)
    {
        ...
        foreach (var d in _registry.Descriptors)                  // N 次查詢,M4 實測後再決定是否優化
            hits.AddRange(await d.QueryHitsAsync(_db, ids, ct));
        return hits;
    }
    ...
    public async Task<IReadOnlyDictionary<Guid, int>> GetUsageCountsAsync(CancellationToken ct = default)
    {
        ...
        {
            var counts = await d.CountUsageAsync(_db, ct);
    ...
    private async Task<List<Guid>> ResolveLabelIdsAsync(string labelName, bool includeDescendants, CancellationToken ct)
    {
        ...
        var root = all.FirstOrDefault(l => l.Name == labelName);
    ...
    private async Task<List<Label>> GetOrCreateLabelsAsync(string[] labelNames)
    {
        var result = new List<Label>();
        foreach (var name in labelNames.Where(n => !string.IsNullOrWhiteSpace(n)).Distinct())
        {
            var label = await _db.Set<Label>().FirstOrDefaultAsync(l => l.Name == name);
            if (label is null)
            {
                ...
                catch (DbUpdateException)
                {
                    ...
                    label = await _db.Set<Label>().FirstAsync(l => l.Name == name);
                }
            }
            result.Add(label);
        }
